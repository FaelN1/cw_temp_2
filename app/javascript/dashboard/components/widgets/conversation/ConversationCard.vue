...
export default {
  ...
  computed: {
    ...
    currentFunnel() {
      if (!this.kanbanItem?.funnel_id || !this.funnels.length) return null;
      return this.funnels.find(
        funnel => funnel.id === this.kanbanItem.funnel_id
      );
    },
    kanbanStageLabel() {
      if (!this.kanbanItem || !this.currentFunnel?.stages) return null;

      const stage = this.currentFunnel.stages[this.kanbanItem.funnel_stage];
      return stage?.name || this.kanbanItem.funnel_stage;
    },

    kanbanTagClasses() {
      if (!this.kanbanItem || !this.currentFunnel?.stages) return '';
    ...
    isKanbanEnabled() {
      return this.isFeatureEnabledonAccount(this.accountId, 'kanban_board');
    },

    showKanbanInfo() {
      return this.isKanbanInfoVisible && this.kanbanItem && this.currentFunnel;
    },
  },
  watch: {
    'chat.id': {
      immediate: true,
      handler() {
        if (this.isKanbanEnabled) {
          this.isKanbanInfoVisible = Boolean(
            this.kanbanItem && this.funnels.length > 0
          );
        }
      },
    },
  ...
  >
    ...
    >
      ...
      <div class="absolute flex flex-col conversation--meta right-4 top-4">
        ...
        <span
          class="unread shadow-lg rounded-full hidden text-xxs font-semibold h-4 leading-4 ml-auto mt-1 min-w-[1rem] px-1 py-0 text-center text-orange-600 bg-green-400"
        >
      ...
      <div v-if="showKanbanInfo" class="flex gap-1 mt-2 ml-2">
        <span
          class="text-xs font-medium px-2 py-0.5 rounded-full"
          :style="kanbanFunnelStyle"
        >
          {{ currentFunnel.name }}
        </span>
        <span
          class="text-xs font-medium px-2 py-0.5"
          :class="kanbanTagClasses"
