...
class Account < ApplicationRecord
  ...
  # Métodos temporários para atributos Stripe
  def stripe_customer_id
    respond_to?(:read_attribute, true) ? read_attribute(:stripe_customer_id) : nil
  end

  def stripe_customer_id=(value)
    write_attribute(:stripe_customer_id, value) if respond_to?(:write_attribute, true)
  end

  def stripe_subscription_id
    respond_to?(:read_attribute, true) ? read_attribute(:stripe_subscription_id) : nil
  end

  def stripe_subscription_id=(value)
    write_attribute(:stripe_subscription_id, value) if respond_to?(:write_attribute, true)
  end

  def stripe_price_id
    respond_to?(:read_attribute, true) ? read_attribute(:stripe_price_id) : nil
  end

  def stripe_price_id=(value)
